...
describe("portable project validation", () => {
  ...
  it("upgrades projects saved before the brief-aligned model", () => {
    const legacy = {
      creative: { ...sample, priorities: { brand: 30, headline: 100, image: 70, price: 50, cta: 100 } },
    ...
    expect(creative.priorities).toEqual({ brand: 4, headline: 1, image: 2, price: 3, cta: 1 });
  ...
  it("migrates legacy projects whose headline priority is not above 5", () => {
    ...
    const { creative } = parseProject({
      creative: { ...sample, priorities: { headline: 1, image: 80, cta: 100, brand: 40, price: 60 } },
      surface: legacySurface,
    });
    expect(creative.priorities).toEqual({ headline: 5, image: 2, cta: 1, brand: 4, price: 3 });
    // Version 1 exports are legacy even when every priority happens to be 1–5.
    const v1 = parseProject({ version: 1, creative: { ...sample, priorities: { headline: 5, image: 3, cta: 5, brand: 1, price: 2 } }, surface: legacySurface });
  ...
  it("leaves current-schema projects untouched and converts out-of-range priorities", () => {
    const current = { ...sample, priorities: { headline: 5, image: 5, cta: 5, brand: 5, price: 5 } };
    expect(parseProject({ version: 2, creative: current, surface: surfaces[0] }).creative).toEqual(current);
